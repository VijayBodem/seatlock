...
import { createPaymentForHold } from '../services/payment.service'
import { createHold } from '../services/hold.service'
import { getShowtime, getShowtimeSeats } from '../services/showtime.service'
import type {
  Booking,
  PaymentSession,
  SeatHold,
...
function getAvailableSeatTypeClasses(type: ShowtimeSeat['type']): string {
  if (type === 'PREMIUM') {
...
function getSeatClasses(seat: ShowtimeSeat, isSelected: boolean): string {
  if (isSelected) {
...
function Seat({
  ...
  const isAvailable = seat.status === 'AVAILABLE' && !isSelectionDisabled

  const statusLabel = isSelected
    ? t('showtimeSeats.status.selected')
    : t(`showtimeSeats.status.${seat.status.toLowerCase()}`)

  const typeLabel = t(`showtimeSeats.type.${seat.type.toLowerCase()}`)
  ...
  return (
    ...
    >
      ...
      {seat.type === 'ACCESSIBLE' && seat.status === 'AVAILABLE' && !isSelected && (
        <span
          aria-hidden="true"
...
export function ShowtimeSeatsPage() {
  ...
  const { accessToken, isAuthenticated, isRestoring } = useAuth()
  ...
  const [remainingHoldMs, setRemainingHoldMs] = useState(0)
  ...
  const handleRealtimeSeatStatusChanged = useCallback((event: SeatStatusChangedEvent) => {
    const changedSeatIds = new Set(event.seatIds)

    setSeats((currentSeats) =>
      currentSeats.map((seat) => {
        if (!changedSeatIds.has(seat.seatId)) {
          return seat
        }

        if (seat.status === event.status) {
          return seat
        }
    ...
    if (event.status !== 'AVAILABLE') {
      setSelectedSeatIds((currentSeatIds) =>
        currentSeatIds.filter((seatId) => !changedSeatIds.has(seatId)),
      )
    }
  }, [])
  ...
  useShowtimeRealtime({
    ...
    onSeatStatusChanged: handleRealtimeSeatStatusChanged,
  })
  ...
  useEffect(() => {
    ...
    function updateRemainingTime() {
      ...
      setRemainingHoldMs(Math.max(0, remaining))
    }
  ...
  const bookingSeats = useMemo(() => {
    ...
    return seats
      .filter((seat) => booking.seatIds.includes(seat.seatId))
      .sort((left, right) => {
        const rowDifference = left.row.localeCompare(right.row)
  ...
  const hasActiveHold = hold !== null && booking === null && remainingHoldMs > 0

  const hasExpiredHold = hold !== null && booking === null && remainingHoldMs <= 0

  const isSelectionDisabled = hold !== null || booking !== null

  function handleSeatToggle(seat: ShowtimeSeat) {
    if (seat.status !== 'AVAILABLE' || isSelectionDisabled) {
      return
    }
    ...
    setSelectedSeatIds((currentSeatIds) => {
      if (currentSeatIds.includes(seat.seatId)) {
        return currentSeatIds.filter((seatId) => seatId !== seat.seatId)
      }

      return [...currentSeatIds, seat.seatId]
    })
  }
  ...
  async function handleCreateHold() {
    if (!accessToken || selectedSeatIds.length === 0 || hold) {
      return
    }
    ...
    try {
      const createdHold = await createHold(
        showtimeId,
        {
      ...
      setRemainingHoldMs(Math.max(0, new Date(createdHold.expiresAt).getTime() - Date.now()))

      setSeats((currentSeats) =>
        currentSeats.map((seat) =>
          createdHold.seatIds.includes(seat.seatId)
            ? {
                ...seat,
                status: 'HELD',
              }
      ...
      if (error instanceof ApiError && error.status === 409) {
        setHoldError(t('showtimeSeats.hold.conflict'))
      } else if (error instanceof ApiError && error.status === 401) {
        setHoldError(t('showtimeSeats.hold.sessionExpired'))
      } else {
        setHoldError(t('showtimeSeats.hold.genericError'))
      }
    } finally {
  ...
  async function handleCreatePayment() {
    if (!accessToken || !hold || remainingHoldMs <= 0 || paymentSession || isCreatingPayment) {
      return
    }
    ...
    try {
      const createdPayment = await createPaymentForHold(hold.id, accessToken)

      setPaymentSession(createdPayment)
    } catch (error) {
      if (error instanceof ApiError && error.status === 401) {
        setBookingError(t('showtimeSeats.booking.sessionExpired'))
      } else if (error instanceof ApiError && (error.status === 404 || error.status === 409)) {
        setBookingError(t('showtimeSeats.booking.expired'))
      } else {
        setBookingError(t('showtimeSeats.booking.genericError'))
      }
    } finally {
      setIsCreatingPayment(false)
    }
  ...
  async function handlePaymentConfirmed() {
    ...
    try {
      for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        const confirmedBooking = bookings.find((candidate) => candidate.holdId === hold.id)
        ...
        if (confirmedBooking) {
          ...
          setSeats((currentSeats) =>
            currentSeats.map((seat) =>
              confirmedBooking.seatIds.includes(seat.seatId)
                ? {
                    ...seat,
                    status: 'BOOKED',
                  }
      ...
      setBookingError(t('showtimeSeats.payment.bookingProcessing'))
    } catch (error) {
      if (error instanceof ApiError && error.status === 401) {
        setBookingError(t('showtimeSeats.booking.sessionExpired'))
      } else {
        setBookingError(t('showtimeSeats.payment.bookingVerificationPending'))
      }
    }
  }

  const formattedShowtime = showtime
    ? formatShowtime(showtime.startsAt, i18n.resolvedLanguage ?? i18n.language)
    : null
  ...
  return (
    <main className="min-h-[calc(100vh-72px)] bg-zinc-50 dark:bg-zinc-950">
      <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        {isLoading && (
          <div className="mt-8 space-y-6" aria-label={t('showtimeSeats.loading')}>
              ...
              <div className="mt-2 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  {showtime.venue.address && (
                    <p className="mt-1 text-sm text-zinc-500">{showtime.venue.address}</p>
                  )}
                </div>

                <dl className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-4 lg:grid-cols-2">
                  <div>
                    <dt className="font-bold text-zinc-500">{t('showtimeSeats.date')}</dt>
                  ...
                  <div>
                    <dt className="font-bold text-zinc-500">{t('showtimeSeats.time')}</dt>
                    <dd className="mt-1 font-bold text-zinc-950 dark:text-white">
                  ...
                  <div>
                    <dt className="font-bold text-zinc-500">{t('showtimeSeats.screen')}</dt>
                    <dd className="mt-1 font-bold text-zinc-950 dark:text-white">
                  ...
                  <div>
                    <dt className="font-bold text-zinc-500">{t('showtimeSeats.available')}</dt>
                    <dd className="mt-1 font-bold text-zinc-950 dark:text-white">
                      {availableSeatCount}
                    </dd>
                  </div>
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  <div className="mt-10 overflow-x-auto pb-3">
                    <div className="mx-auto w-max min-w-full space-y-4">
                      {seatsByRow.map(({ row, seats: rowSeats }) => (
                        <div className="flex min-w-max items-center justify-center gap-3" key={row}>
                  ...
                  <div className="mt-10 border-t border-zinc-200 pt-6 dark:border-zinc-800">
                    <div>
                      <p className="text-center text-xs font-extrabold tracking-[0.14em] text-zinc-500 uppercase">
                        {t('showtimeSeats.legend.seatTypes')}
                      </p>
                      ...
                      <div className="mt-3 flex flex-wrap justify-center gap-x-6 gap-y-3 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-zinc-600 dark:text-zinc-400">
                            {t('showtimeSeats.type.accessible')}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      <p className="text-center text-xs font-extrabold tracking-[0.14em] text-zinc-500 uppercase">
                        {t('showtimeSeats.legend.seatStatus')}
                      </p>
              ...
              <aside className="h-fit rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm xl:sticky xl:top-24 dark:border-zinc-800 dark:bg-zinc-900">
                <h2 className="text-lg font-black text-zinc-950 dark:text-white">
                  {booking ? t('showtimeSeats.booking.title') : t('showtimeSeats.selection.title')}
                </h2>
                ...
                {booking && (
                  <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950/40">
                    <p className="text-sm font-black text-emerald-900 dark:text-emerald-200">
                      {t('showtimeSeats.booking.success')}
                    </p>

                    <p className="mt-2 text-sm text-emerald-700 dark:text-emerald-300">
                      {t('showtimeSeats.booking.number', {
                        id: booking.id,
                      })}
                    </p>
                ...
                {hasExpiredHold && (
                  <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/40">
                    ...
                    <button
                      ...
                      onClick={handleRefreshAfterExpiry}
                      type="button"
                    >
                      {t('showtimeSeats.hold.refreshSeats')}
                    </button>
                  </div>
                )}

                {!hold && !booking && selectedSeats.length > 0 && (
                  <div className="mt-5">
                    <p className="text-sm font-bold text-zinc-500">
                      {t('showtimeSeats.selection.selected', {
                ...
                {!hold && !booking && selectedSeats.length === 0 && (
                  <p className="mt-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                    {t('showtimeSeats.selection.empty')}
                  </p>
                )}
                ...
                {!hold && !booking && !isRestoring && isAuthenticated && (
                  <button
                    ...
                    disabled={selectedSeatIds.length === 0 || isCreatingHold}
                    onClick={() => void handleCreateHold()}
                    type="button"
                  >
                    {isCreatingHold
                      ? t('showtimeSeats.hold.creating')
                      : t('showtimeSeats.hold.create')}
                  </button>
                )}

                {hasActiveHold && isAuthenticated && !paymentSession && (
                  <button
                  ...
                  >
                    {isCreatingPayment
                      ? t('showtimeSeats.payment.preparing')
                      : t('showtimeSeats.payment.continue')}
                  </button>
                )}

                {hasActiveHold && isAuthenticated && paymentSession && stripeElementsOptions && (
                  <Elements options={stripeElementsOptions} stripe={stripePromise}>
                    <StripeCheckout
                      isHoldActive={remainingHoldMs > 0}
